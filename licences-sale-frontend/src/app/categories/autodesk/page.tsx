...
const CATEGORY_INFO = {
	name: 'Autodesk',
	slug: 'autodesk',
	description: 'Logiciels de conception et modélisation 3D professionnels. AutoCAD, Revit, 3ds Max et toute la suite Autodesk.',
	...
	subcategories: [
		{ name: 'AutoCAD 2025', query: 'AutoCAD' },
		{ name: 'Revit 2025', query: 'Revit' },
		{ name: '3ds Max', query: '3ds Max' },
		{ name: 'Maya', query: 'Maya' },
		{ name: 'Inventor', query: 'Inventor' },
		{ name: 'Civil 3D', query: 'Civil 3D' },
	],
};
...
export default function AutodeskPage() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				setLoading(true);
				const query = selectedSubcategory || 'Autodesk';
					...
					<div className="flex items-center justify-center min-h-[60vh]">
						<div className="text-center">
							<div className="relative">
								<div className="w-20 h-20 rounded-full border-4 border-gray-200 border-t-cyan-600 animate-spin mx-auto" />
								<Loader2 size={32} className="text-cyan-600 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
							</div>
				...
				<section className="py-8 bg-white border-b border-gray-100">
					<div className="max-w-7xl mx-auto px-6">
						<div className="flex flex-wrap items-center gap-3">
							...
							<button type="button" onClick={() => setSelectedSubcategory(null)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${!selectedSubcategory ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
								Tous
							</button>
							{CATEGORY_INFO.subcategories.map((sub) => (
								<button key={sub.name} type="button" onClick={() => setSelectedSubcategory(sub.query)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedSubcategory === sub.query ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
									{sub.name}
								</button>
							))}
						</div>
					...
					<div className="max-w-7xl mx-auto px-6">
						<div className="bg-white rounded-2xl p-4 lg:p-5 shadow-sm border border-gray-100 mb-8">
							<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
								<div className="flex items-center gap-4">
									<button type="button" onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium transition-all ${showFilters ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
								...
								<div className="flex items-center gap-3">
									...
									<div className="flex items-center bg-gray-100 rounded-xl p-1">
										<button type="button" onClick={() => setViewMode('grid')} className={`p-2.5 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-white shadow-sm text-cyan-600' : 'text-gray-400'}`}><Grid3X3 size={18} /></button>
										<button type="button" onClick={() => setViewMode('list')} className={`p-2.5 rounded-lg transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-cyan-600' : 'text-gray-400'}`}><LayoutList size={18} /></button>
									</div>
								</div>
							</div>
										...
										<div>
											...
											<div className="flex flex-wrap gap-2">
												{[{ label: 'Tous', range: [0, 1000000] }, { label: '< 100K', range: [0, 100000] }, { label: '100K - 300K', range: [100000, 300000] }, { label: '> 300K', range: [300000, 1000000] }].map((option) => (
													<button key={option.label} type="button" onClick={() => setPriceRange(option.range as [number, number])} className={`px-3 py-1.5 text-sm rounded-lg transition-colors ${priceRange[0] === option.range[0] && priceRange[1] === option.range[1] ? 'bg-cyan-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{option.label}</button>
												))}
											</div>
										</div>
						...
						{filteredProducts.length > 0 ? (
									...
									return viewMode === 'grid' ? (
											...
											<div className={`relative bg-white rounded-2xl overflow-hidden border transition-all duration-300 ${isHovered ? 'border-transparent shadow-2xl -translate-y-2' : 'border-gray-100 shadow-sm'}`}>
												<div className="relative h-56 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
													...
													<div className={`absolute top-3 right-3 z-10 flex flex-col gap-2 transition-all duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
														...
														<Link href={`/products/${product.slug}`} className="w-9 h-9 bg-white rounded-lg shadow-md flex items-center justify-center text-gray-400 hover:text-cyan-600"><Eye size={18} /></Link>
													</div>
												...
												<div className="p-5">
													...
													<Link href={`/products/${product.slug}`}><h3 className="text-lg font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-cyan-600 transition-colors">{product.name}</h3></Link>
										...
										<div key={product.id} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all flex flex-col sm:flex-row">
											...
											<div className="flex-1 p-6 flex flex-col justify-center">
												...
												<Link href={`/products/${product.slug}`}><h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-cyan-600">{product.name}</h3></Link>
							...
							<div className="text-center py-20">
								...
								<button type="button" onClick={() => { setPriceRange([0, 1000000]); setSelectedSubcategory(null); }} className="px-6 py-3 bg-cyan-600 text-white font-semibold rounded-xl">Réinitialiser</button>
							</div>
						)}
					</div>
				</section>
